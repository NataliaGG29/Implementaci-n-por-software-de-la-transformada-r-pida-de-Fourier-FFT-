...
def DatosA():
    ...
    while (isRun):
        ...
        #    isReceive = False
        #else:
            datos = float(arduino.readline().decode())
            isReceive = True
        except:
            isReceive = True
...
plt.title("Grafica (0 - 200x10^3) magnitud", color='red', size=16, family="Tahoma")
...
label = Label(frame, textvariable=labelx, bg="#5CFE05", fg="black", font="Helvetica 13 bold", width=11,
              justify="center")
label.grid(row=0, column=0, padx=5, ipady=8, pady=10)

Iniciar = Button(frame, command=Iniciar, text="Iniciar ", bg="blue", fg="white", font="Helvetica 14 bold", width=9,
                 justify="center")
...
limpiar = Button(frame, command=Limpiar, text="Limpiar ", bg="blue", fg="white", font="Helvetica 14 bold", width=9,
                 justify="center")
